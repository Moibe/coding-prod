...
case'parallax_effect_fixed':parallaxType='parallax_fixes';bufferRatio=1;break
...
if(parallaxType=='parallax_fixes'){if(ieVersion!=-1){parallax=false;obj_inner.css({backgroundAttachment:'fixed'});}}
originalWidth=img.width;originalHeight=img.height;objectResize(obj_inner,primaryImageHolder.width(),_baseHeight);onSlideLoaded();}}
...
if(ieVersion<10){mainImageHolder.css({"top":newPositionTop});}else{mainImageHolder.css({"transform":"translate3d(0px, "+ newPositionTop+"px, 0px)"});}}
if(options.scrolling_description){description_opacity=(1-(y/_thisHeight)).toFixed(2);if(description_opacity<1)mainCaptionHolder.css('opacity',description_opacity);description_offset=parseInt(y/2.5);if(description_offset>0){if(ieVersion<10){mainCaptionHolder.css({"top":description_offset});}else{mainCaptionHolder.css({"transform":"translate3d(0px, "+ description_offset+"px, 0px)"});}}else{if(ieVersion<10){mainCaptionHolder.css({"top":0});}else{mainCaptionHolder.css({"transform":"translate3d(0px, 0px, 0px)"});}}}}}}
...
function getBaseHeight(){console.log(parallaxType);switch(parallaxType){case'parallax_normal':if(_thisOffsetTop<(_windowHeight-_thisHeight)){baseHeight=_thisHeight+ parseInt(_thisOffsetTop/bufferRatio);}else{baseHeight=_thisHeight+ parseInt((_windowHeight- _thisHeight)/bufferRatio);
}
break
